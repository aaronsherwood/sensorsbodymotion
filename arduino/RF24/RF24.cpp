...
#ifdef __arm__  // define PROGMEM macro as blank for non-AVR chips

#define RF24_PRINTF_BUFSZE 128

int RF24_printf( char * fmt, ...) 
{
  static char fbuf[RF24_PRINTF_BUFSZE];
  va_list args;
  va_start (args, fmt);
  vsnprintf(fbuf, sizeof(fbuf), fmt, args); // does not overrun sizeof(buf) including null terminator
  va_end (args);

  int n = strlen(fbuf); // no. of chars written to formatted string

  if (n) Serial.print(fbuf); // send to serial out

  return n; // return char count
} 

#define printf RF24_printf
