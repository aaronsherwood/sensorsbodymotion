...
//
int newVideoResolutionY;
PImage cropped;
int topMargin;

void setup() {
  size(1280, 480);
  float aspectRatio = 1280f/800f; //just for example
  newVideoResolutionY = int(videoWidth/aspectRatio);
  println(newVideoResolutionY);
  topMargin = (videoHeight-newVideoResolutionY)/2;
  cropped = createImage(videoWidth, newVideoResolutionY, ARGB); 

  warped = createImage(videoWidth, newVideoResolutionY, ARGB); 
  loadData();
...
void draw() {
  ...
  if (video.available()) {
    video.read();
    //no need to cropped i realized
    //cropped.copy(video, 0, topMargin, videoWidth, newVideoResolutionY, 0, 0, videoWidth, newVideoResolutionY);
    //just load the original video in
    opencv.loadImage(video);
    //but when we warp we still need to give the new Y resolution based on our projectpr's aspect ratio
    opencv.toPImage(warpPerspective(vectors, videoWidth, newVideoResolutionY), warped);
  }
  image(video, 0, 0);
